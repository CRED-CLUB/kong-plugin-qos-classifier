...
local function set_classes_with_thresholds(green, orange, red)
  classes = {
    class_1 = {threshold = green, header_value = "Green"},
    class_2 = {threshold = orange, header_value = "Orange"},
    class_3 = {threshold = red, header_value = "Red"}
  }
  return classes
end
...
local function make_requests_to_upstream(host, start_time)
  ...
  while true do
    ...
    assert(client:get("/get", {headers = {Host = host}}))
    ...
    -- of requests received in the previous seconds.
    -- It takes 2 seconds for the plugin to detect QoS levels
    -- Therefore, 2.2 seconds is used as a window
    if (ngx.now() - start_time > 2.2) then break end
  end
end
...
for _, strategy in helpers.each_strategy() do
  describe(PLUGIN_NAME .. ": (integration) [#" .. strategy .. "]", function()
    ...
    setup(function()
      ...
      local route2 = bp.routes:insert{hosts = {RED_HOST}, service = service}

      local route3 = bp.routes:insert{hosts = {ORANGE_HOST}, service = service}

      local route4 = bp.routes:insert{hosts = {GREEN_HOST}, service = service}
    ...
    it("Check for termination", function()
      ...
      local res = client:get("/get", {headers = {Host = TERMINATE_HOST}})
    ...
    it("Check Red Header", function()
      ...
      local client = helpers.proxy_client()
      local res = client:get("/get", {headers = {Host = RED_HOST}})
    ...
    it("Check Orange Header", function()
      ...
      local client = helpers.proxy_client()
      local res = client:get("/get", {headers = {Host = ORANGE_HOST}})
      body = assert.res_status(200, res)
      json = cjson.decode(body)
    ...
    it("Check Green Header", function()
      ...
      local client = helpers.proxy_client()
      local res = client:get("/get", {headers = {Host = GREEN_HOST}})
      body = assert.res_status(200, res)
      json = cjson.decode(body)
