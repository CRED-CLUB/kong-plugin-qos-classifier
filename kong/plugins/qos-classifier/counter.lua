...
do
  ...
  if not ok then
    clear_tab = function(tab) for k in pairs(tab) do tab[k] = nil end end
  end
end
...
local function sync(_, self)
  ...
  for k, v in pairs(self.increments) do
    _, err, _ = self.dict:incr(k, v, 0, config.COUNTER_TTL_IN_SECS)
    if err then
      ngx.log(ngx.WARN, "error increasing counter in shdict key: ", k,
              ", err: ", err)
      ok = false
    end
  end
...
function _M:sync() return sync(false, self) end

function _M:incr(key, step)
  step = step or 1
  local v = self.increments[key]
  if v then step = step + v end
...
function _M:reset(key, number)
  if not number then return nil, "expect a number at #2" end
  return self.dict:incr(key, -number, number)
end

function _M:get(key) return self.dict:get(key) end

function _M:get_keys(max_count) return self.dict:get_keys(max_count) end
